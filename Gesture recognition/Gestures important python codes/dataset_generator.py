# This helps to generate a custom dataset
# It asks for a gesture, then takes a lot of pictures
...
# Augmentations can also be added here directly
...
# Will not overwrite images that already exists, it will start after the last image
...
# Can be set to only capture specific gestures (by lack of data)
def capture_gestures(output_dir='my_dataset', gestures=['04_left', '05_right', '06_forward', '07_backward', '08_nothing'], samples_per_gesture=800):
    ...
    # in the resolution that will actually used by the webcam
    cap.set(cv2.CAP_PROP_FRAME_WIDTH, 320)
    cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 180)

    # Loads the model, because it will show the gesture predicted. It can also capture only when the recognized gesture is wrong for further training
    ...
    for gesture in gestures:
        ...
        while True:
            ...
            # This will make it black and white, in order to test the ML model
            # Apply Gaussian blur
        ...
        # This is to only write files if the predicted gesture is wrong
